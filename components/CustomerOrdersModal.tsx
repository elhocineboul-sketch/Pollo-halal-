    ...
    <Modal isOpen={isOpen} onClose={onClose} title={t('customerOrdersTitle', { customerName: customer.name })}>
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2"> {/* Added pr-2 for scrollbar spacing */}
        {customer.orders.length === 0 ? (
          <p className="text-center text-gray-500 dark:text-gray-400">{t('noOrdersFoundMessage')}</p>
        ) : (
          customer.orders.map((order) => (
            <div key={order.id} className="bg-gray-50 p-4 rounded-xl shadow-sm text-end dark:bg-gray-700 dark:text-white"> {/* Changed text-right to text-end */}
              ...
              <div className="flex justify-between items-center mb-3 pb-2 border-b border-gray-200 dark:border-gray-600">
                <span className="text-gray-600 text-sm font-medium dark:text-gray-300">{t('orderDateTableHeading')}:</span>
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-xl font-extrabold text-amber-700 dark:text-amber-200">${order.totalAmount.toFixed(2)}</span>
              </div>
              ...
              <div className="space-y-2">
                {order.items.map((item: CartItem) => (
                  <div key={item.product.id} className="flex items-center gap-3 bg-white p-2 rounded-lg shadow-xs dark:bg-gray-800">
                    ...
                    <div className="flex-1 text-end"> {/* Changed text-right to text-end */}
                      <div className="font-medium text-xs dark:text-white">{item.product.name[locale] || item.product.name.es || item.product.name.en || 'N/A'}</div>
                      <div className="text-gray-500 text-xs dark:text-gray-400">{t('quantity')}: {item.quantity} x ${item.product.sale.toFixed(2)}</div>
                    </div>
                    <span className="font-bold text-sm dark:text-white">${(item.quantity * item.product.sale).toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
